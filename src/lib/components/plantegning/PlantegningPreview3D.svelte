...
<!-- src/lib/components/akse/plantegning/PlantegningPreview3D.svelte -->
<script lang="ts">
  ...
  onMount(() => {
    ...
    // Initiell ekstrudering — sikrer at en skisse som åpnes for re-redigering
    // vises umiddelbart, uavhengig av $effect-timingen.
    rebuild(store.toSketchData());
  });
  ...
  // Live, debounced re-ekstrudering. Å lese store.toSketchData() her sporer
  // figurer, dimensjoner og extrudeHeight automatisk. selectedIds leses ikke,
  // så seleksjon trigger ikke re-ekstrudering.
  $effect(() => {
    const data = store.toSketchData();
    if (rebuildTimer !== null) clearTimeout(rebuildTimer);
    rebuildTimer = setTimeout(() => rebuild(data), DEBOUNCE_MS);
  });

  function rebuild(data: SketchData) {
    if (!modelGroup) return; // scenen er ikke satt opp ennå

    let geom: THREE.BufferGeometry | null = null;
    try {
      geom = extrudeSketch(data);
    } catch (e) {
      console.warn('[Plantegning] 3D-preview kunne ikke ekstruderes:', e);
      status = 'error';
      return; // behold forrige gyldige mesh
    }

    if (!geom) {
      // Tom skisse: fjern mesh.
      if (mesh) {
